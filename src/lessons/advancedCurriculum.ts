...
/** Every search must handle a hit, an edge element and a missing value. */
const searchTests = (id: string, moves: string, cases: [string, string][]) => [
  ...tests(id, moves, cases[0][1]).map(test => ({ ...test, input: cases[0][0] })),
  ...cases.slice(1).map(([input, output], index) => ({
    id: `${id}-boundary-${index}`, name: `Tìm mã ${input}: in ${output}`,
    kind: 'output' as const, input, expectedOutput: output, required: true, visible: true,
  })),
];
