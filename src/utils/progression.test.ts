...
describe('progression Area 0–2',()=>{
  ...
  it('chọn đúng Area tiếp theo',()=>{ expect(getNextLessonId({progressByLesson:{}})).toBe('a0'); expect(getNextLessonId({progressByLesson:{a0:progress('a0','completed')}})).toBe('a1'); expect(getNextLessonId({progressByLesson:{a0:progress('a0','completed'),a1:progress('a1','completed'),a2:progress('a2','completed')}})).toBe('a2'); });
  it('tổng sao theo đúng ba Area',()=>{ expect(getTotalStars({a0:progress('a0','completed',3),a1:progress('a1','in_progress',2)})).toBe(5); expect(MAX_TOTAL_STARS).toBe(9); });
});
